            ...
            <form action="{{ route('dashboard.product.update', $item->slug) }}" class="w-full" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex flex-wrap -mx-4 mb-6">
                    ...
                    <div class="w-3/12 px-3">
                        ...
                        <input type="number" min="0" oninput="validity.valid||(value='');" value="{{ old('price') ?? $item->price }}" name="price" placeholder="Input Item Price Here" class="block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                    </div>
                    {{-- input price --}}
                    {{-- input price --}}
                    <div class="w-3/12 px-3">
